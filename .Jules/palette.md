...
## 2026-06-04 - Accessible Keyboard Tooltips and Abbreviation Outlines\n**Learning:** Relying on 'title' tooltip for <kbd> hints and <abbr> definitions is inequitable for keyboard users because native <abbr> tags are not inherently focusable, and sighted users might not infer shortcut functionality without explicit tooltips.\n**Action:** Add 'tabIndex={0}' and focus outlines to <abbr> tags. Additionally, include 'title' attributes on visual <kbd> hints to clarify shortcut functionality for mouse users.
## 2026-06-07 - [Persistent Loading States for Async Actions]
**Learning:** Buttons triggering async network requests often lack persistent visual feedback during the loading phase, which violates UX interaction principles and can lead to duplicate clicks. Simply binding a rotate animation to hover/focus states is insufficient for long-running operations.
**Action:** When creating action buttons (like 'Retry'), bind the application's `loading` state directly to the button by adding `disabled={loading}` and conditionally applying a persistent loading animation (e.g., `animate-spin`) to the icon, replacing any standard hover/focus animations to avoid conflicts.
